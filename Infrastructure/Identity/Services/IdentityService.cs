using MosCore.Application.DTOs.Identity;
using MosCore.Application.DTOs.Mail;
using MosCore.Application.DTOs.Settings;
using MosCore.Application.Enums;
using MosCore.Application.Exceptions;
using MosCore.Application.Interfaces;
...
namespace MosCore.Infrastructure.Identity.Services
{
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _mailService.SendAsync(new MailRequest() { From = "manikgupta@mospido.com", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
